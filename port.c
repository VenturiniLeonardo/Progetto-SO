...
int generatorSupply(){
    ...
    good_d->states[newGood.type-1].goods_in_port += newGood.quantity;
    port_d->states[my_index].goods_offer+=newGood.quantity;
    sops_dump.sem_op=1;
    semop(dumpSem,&sops_dump,1);
...
int generatorDemand(){
    ...
    port_d->states[my_index].goods_demand += msg.quantity; 
    sops_dump.sem_op=1;
    semop(dumpSem,&sops_dump,1);
